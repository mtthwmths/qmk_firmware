...
// entirely and just use numbers. Me, I'm gonna use the DV names to help me remember what they do.
#define _QWERTY 0
#define _LOWER 1
#define _RAISE 2
#define _DVORAK 3
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    ...
    case LOWER:
      if (record->event.pressed) {
        layer_on(_LOWER);
        // I'll forget this again (I've forgotten it before) so let's make a comment.
        // update_tri_layer(x, y, z) basically checks if x & y then z. so there.
